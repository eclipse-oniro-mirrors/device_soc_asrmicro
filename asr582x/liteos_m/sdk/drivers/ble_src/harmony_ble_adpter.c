...
extern void app_ble_start_advertising_with_param(sonata_gap_directed_adv_create_param_t* param,ble_adv_data_set_t *data,ble_scan_data_set_t *scan_data, uint8_t own_addr_type,uint16_t duration, uint8_t max_adv_evt);
...
static void perm_2_perm(uint8_t user_perm,uint8_t user_prop,uint16_t *p_perm)
{
    ...
    {
        if((user_perm & OHOS_GATT_PERMISSION_READ_ENCRYPTED) == OHOS_GATT_PERMISSION_READ_ENCRYPTED || (user_perm & OHOS_GATT_PERMISSION_WRITE_ENCRYPTED) == OHOS_GATT_PERMISSION_WRITE_ENCRYPTED)
        {
            *p_perm |= PNTF_UA;
        }
        else
...
int BleGattsStartServiceEx(int *srvcHandle, BleGattService *srvcInfo)
{
  if(NULL == srvcInfo)
  {
   ...
   if((srvcInfo->attrList[att_nb - 1].properties  & OHOS_GATT_CHARACTER_PROPERTY_BIT_INDICATE) == OHOS_GATT_CHARACTER_PROPERTY_BIT_INDICATE 
      ||  (srvcInfo->attrList[att_nb - 1].properties  & OHOS_GATT_CHARACTER_PROPERTY_BIT_NOTIFY) == OHOS_GATT_CHARACTER_PROPERTY_BIT_NOTIFY )
   {
        indicate_is_exit = 1;
   }
   ...
   for(int i = 0; i < att_nb; i++)
   {
       ...
       print_log("uudi %x %x %x %x",att_list[i].att_desc.uuid[0],att_list[i].att_desc.uuid[1],att_list[i].att_desc.uuid[2],att_list[i].att_desc.uuid[3]);
   }
