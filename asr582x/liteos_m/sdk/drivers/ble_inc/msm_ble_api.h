...
#define PWC_AU   (PERM(WRITE_COMMAND, ENABLE) | PERM(WP, AUTH))
...
#define PWC_UA   (PERM(WRITE_COMMAND, ENABLE) | PERM(WP, UNAUTH))
...
typedef enum MS_BLE_STACK_EVENT_T
{
    ...
    MS_BLE_STACK_EVENT_ADV_ON,                  /*!< 开启或关闭广播执行成功*/
    MS_BLE_STACK_EVENT_ADV_OFF,                /*!< 开启或关闭广播执行成功*/
