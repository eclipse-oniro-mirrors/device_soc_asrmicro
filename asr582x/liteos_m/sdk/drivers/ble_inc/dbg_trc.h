 ...
 ****************************************************************************************
 */
//modified for rom 20200226
//void dbg_trc_req_ke_msg_send(struct ke_msg const *msg);
extern void (*pf_dbg_trc_req_ke_msg_send)(struct ke_msg const *msg);
extern void (*pf_dbg_trc_req_ke_msg_handled)(ke_msg_id_t const id, ke_task_id_t const dest_id, ke_task_id_t const src_id, uint8_t const msg_status);
...
#endif
#define TRC_REQ_KE_MSG_SEND(msg) \
        if(pf_dbg_trc_req_ke_msg_send)        \
        {                                     \
            pf_dbg_trc_req_ke_msg_send(msg);  \
        }
...
#endif
#define TRC_REQ_KE_MSG_HANDLED(id, dest_id, src_id, msg_status)             \
        if(pf_dbg_trc_req_ke_msg_handled)                                   \
        {                                                                   \
            pf_dbg_trc_req_ke_msg_handled(id, dest_id, src_id, msg_status); \
        }
...
extern void (*pf_dbg_trc_req_ke_tmr)(uint8_t const tmr_trc_type, uint32_t const target_time, ke_task_id_t const task_id, ke_msg_id_t const msg_id);
#if 0
...
#endif
#define TRC_REQ_KE_TMR_SET(target_time, task_id, msg_id)                  \
	if(pf_dbg_trc_req_ke_tmr)                                             \
    {                                                                     \
		pf_dbg_trc_req_ke_tmr(KE_TMR_SET, target_time, task_id, msg_id);  \
	}        
...
#define TRC_REQ_KE_TMR_CLR(target_time, task_id, msg_id)                 \
	if(pf_dbg_trc_req_ke_tmr)                                            \
	{                                                                    \
	    pf_dbg_trc_req_ke_tmr(KE_TMR_CLR,target_time, task_id, msg_id);  \
	}
...
#define TRC_REQ_KE_TMR_EXP(target_time, task_id, msg_id)                 \
	if(pf_dbg_trc_req_ke_tmr)                                            \
	{	                                                                 \
		pf_dbg_trc_req_ke_tmr(KE_TMR_EXP,target_time, task_id, msg_id);  \
	}        
...
extern void (*pf_dbg_trc_req_ke_evt)(uint8_t const evt_trc_type, uint8_t const event_type);
#if 0
...
/// Macro used to send trace request API for kernel event set
#define TRC_REQ_KE_EVT_SET(evt_type)                    \
	if(pf_dbg_trc_req_ke_evt)                           \
	{                                                   \
	    pf_dbg_trc_req_ke_evt(KE_EVT_SET, evt_type);    \
	}
...
#define TRC_REQ_KE_EVT_HANDLED(evt_type)                 \
	if(pf_dbg_trc_req_ke_evt)                            \
	{                                                    \
	    pf_dbg_trc_req_ke_evt(KE_EVT_HANDLED, evt_type); \
	}
//modified for rom 20200226
...
extern void (*pf_dbg_trc_req_mem)(uint8_t const mem_trc_type, uint8_t const heap_id, void const *blk_address, uint32_t const size);
#if 0
...
/// Macro used to send trace request API for memory allocation
#define TRC_REQ_MEM_ALLOC(heap_id, blk_address, size)                  \
	    if(pf_dbg_trc_req_mem)                                         \
	    {                                                              \
	        pf_dbg_trc_req_mem(MEM_ALLOC, heap_id, blk_address, size); \
	    }        
...
#define TRC_REQ_MEM_FREE(heap_id, blk_address, size)                   \
	    if(pf_dbg_trc_req_mem)                                         \
	    {                                                              \
	        pf_dbg_trc_req_mem(MEM_FREE, heap_id, blk_address, size);  \
	    }
// modified for rom 20200226
...
extern void (*pf_dbg_trc_req_et_prog)(uint8_t const elt_idx);
#if 0
#define TRC_REQ_ET_PROG(elt_idx) \
        dbg_trc_req_et_prog(elt_idx);
#endif
#define TRC_REQ_ET_PROG(elt_idx)              \
	    if(pf_dbg_trc_req_et_prog)            \
	    {                                     \
	        pf_dbg_trc_req_et_prog(elt_idx);  \
	    }        
// modified for rom 20200226
...
extern void (*pf_dbg_trc_req_evt_cnt)(uint16_t const conhdl, uint16_t const value);
#if 0
#define TRC_REQ_EVT_CNT(conhdl, value) \
        dbg_trc_req_evt_cnt(conhdl, value);
#endif
#define TRC_REQ_EVT_CNT(conhdl, value)          \
	if(pf_dbg_trc_req_evt_cnt)                  \
	{                                           \
	    pf_dbg_trc_req_evt_cnt(conhdl, value);  \
	}
// modified for rom 20200226
...
extern void (*pf_dbg_trc_req_frm_cmp)(uint32_t const timestamp, uint8_t const frm_id, uint32_t const frm_cbk);
#if 0
#define TRC_REQ_FRM_CMP(timestamp, frm_id, frm_cbk) \
        dbg_trc_req_frm_cmp(timestamp, frm_id, frm_cbk);
#endif
#define TRC_REQ_FRM_CMP(timestamp, frm_id, frm_cbk)          \
	if(pf_dbg_trc_req_frm_cmp)                               \
	{                                                        \
	    pf_dbg_trc_req_frm_cmp(timestamp, frm_id, frm_cbk);  \
	}        
// modified for rom 20200226
...
extern void (*pf_dbg_trc_req_cs_ble)(uint16_t const cs_addr);
#if 0
#define TRC_REQ_CS_BLE(cs_addr) \
        dbg_trc_req_cs_ble(cs_addr);
#endif
#define TRC_REQ_CS_BLE(cs_addr)          \
	if(pf_dbg_trc_req_cs_ble)            \
	{                                    \
	    pf_dbg_trc_req_cs_ble(cs_addr);  \
	}
...
extern void (*pf_dbg_trc_req_rx_desc)(uint8_t const drv_type, uint8_t const act_id, uint16_t const rx_desc_add);
...
#endif
#define TRC_REQ_RX_DESC(drv_type, act_id, rx_desc_add)              \
	    if(pf_dbg_trc_req_rx_desc)                                  \
	    {                                                           \
	        pf_dbg_trc_req_rx_desc(drv_type, act_id, rx_desc_add);  \
	    }        
// modified for rom 20200226
...
extern void (*pf_dbg_trc_req_llcp)(uint8_t const llcp_trc_type, uint16_t const conhdl, uint8_t const data_len, uint8_t const *pdu_data);
...
/// Macro used to send trace request API when an llcp packet is transmitted
#define TRC_REQ_LLCP_TX(conhdl, data_len, pdu_data)                    \
	    if(pf_dbg_trc_req_llcp)                                        \
	    {                                                              \
	        pf_dbg_trc_req_llcp(LLCP_TX, conhdl, data_len, pdu_data);  \
	    }        
...
#define TRC_REQ_LLCP_RX(conhdl, data_len, pdu_data)                    \
	    if(pf_dbg_trc_req_llcp)                                        \
	    {                                                              \
	        pf_dbg_trc_req_llcp(LLCP_RX, conhdl, data_len, pdu_data);  \
	    }        
...
#define TRC_REQ_LLCP_ACK(conhdl, data_len, pdu_data)                    \
	    if(pf_dbg_trc_req_llcp)                                         \
	    {                                                               \
	        pf_dbg_trc_req_llcp(LLCP_ACK, conhdl, data_len, pdu_data);  \
	    }       
...
extern void (*pf_dbg_trc_req_sch_arb)(uint8_t const sch_arb_trc_type, const struct sch_arb_elt_tag *sch_arb_elt);
...
/// Macro used to send trace request API when an event is cancelled
#define TRC_REQ_SCH_ARB_CANC(sch_arb_elt)                       \
	    if(pf_dbg_trc_req_sch_arb)                              \
	    {                                                       \
	        pf_dbg_trc_req_sch_arb(SCH_ARB_CANC, sch_arb_elt);  \
	    }        
...
#define TRC_REQ_SCH_ARB_REM(sch_arb_elt)                       \
	    if(pf_dbg_trc_req_sch_arb)                             \
	    {                                                      \
	        pf_dbg_trc_req_sch_arb(SCH_ARB_REM, sch_arb_elt);  \
	    }        
...
#define TRC_REQ_SCH_ARB_START(sch_arb_elt)                       \
	    if(pf_dbg_trc_req_sch_arb)                               \
	    {                                                        \
	        pf_dbg_trc_req_sch_arb(SCH_ARB_START, sch_arb_elt);  \
	    }
...
extern void (*pf_dbg_trc_req_sch_arb_shift)(const struct sch_arb_elt_tag *sch_arb_elt);
#if 0
#define TRC_REQ_SCH_ARB_SHIFT(sch_arb_elt)                \
        dbg_trc_req_sch_arb_shift(sch_arb_elt);
#endif
#define TRC_REQ_SCH_ARB_SHIFT(sch_arb_elt)                \
	    if(pf_dbg_trc_req_sch_arb_shift)                  \
	    {                                                 \
	        pf_dbg_trc_req_sch_arb_shift(sch_arb_elt);    \
	    }        
// modified for rom 20200226
...
extern void (*pf_dbg_trc_req_sch_arb_insert)(uint32_t const target_time, uint8_t const status, const struct sch_arb_elt_tag *sch_arb_elt);
...
#endif
#define TRC_REQ_SCH_ARB_INSERT(target_time, status, sch_arb_elt)              \
	    if(pf_dbg_trc_req_sch_arb_insert)                                     \
	    {                                                                     \
	        pf_dbg_trc_req_sch_arb_insert(target_time, status, sch_arb_elt);  \
	    }
...
extern void (*pf_dbg_trc_req_adv_rx_pdu)(const uint8_t index_pkt, const uint8_t data_len, void const *data);
extern void (*pf_dbg_trc_req_adv_tx_pdu)(const uint8_t adv_txdesc_idx, const uint8_t cs_idx);
extern void (*pf_dbg_trc_req_init_rx_pdu)(const uint8_t index_pkt, uint16_t rxdataptr);
extern void (*pf_dbg_trc_req_init_tx_pdu)(const uint8_t index_pkt, struct bd_addr const addr);
extern void (*pf_dbg_trc_req_scan_rx_pdu)(const uint8_t index_pkt, struct lld_adv_rep_ind const* ind);
extern void (*pf_dbg_trc_req_scan_tx_pdu)(const uint8_t txdesc_idx, const uint8_t cs_idx);
...
#endif
#define TRC_REQ_INIT_RX_PDU(index_pkt, addr)          \
	if(pf_dbg_trc_req_init_rx_pdu)                    \
	{                                                 \
		pf_dbg_trc_req_init_rx_pdu(index_pkt, addr);  \
	}
#define TRC_REQ_INIT_TX_PDU(index_pkt, addr)         \
	if(pf_dbg_trc_req_init_tx_pdu)                   \
	{                                                \
	    pf_dbg_trc_req_init_tx_pdu(index_pkt, addr); \
	}    
#define TRC_REQ_ADV_RX_PDU(index_pkt, data_len, data)         \
	if(pf_dbg_trc_req_adv_rx_pdu)                             \
	{                                                         \
	    pf_dbg_trc_req_adv_rx_pdu(index_pkt, data_len, data); \
	}    
#define TRC_REQ_ADV_TX_PDU(adv_txdesc_idx, cs_idx)          \
	if(pf_dbg_trc_req_adv_tx_pdu)                           \
	{                                                       \
	    pf_dbg_trc_req_adv_tx_pdu(adv_txdesc_idx, cs_idx);  \
	}
#define TRC_REQ_SCAN_RX_PDU(pkt_type, ind)          \
	if(pf_dbg_trc_req_scan_rx_pdu)                  \
	{                                               \
	    pf_dbg_trc_req_scan_rx_pdu(pkt_type, ind);  \
	}    
#define TRC_REQ_SCAN_TX_PDU(txdesc_idx, cs_idx)          \
	if(pf_dbg_trc_req_scan_tx_pdu)                       \
	{	                                                 \
		pf_dbg_trc_req_scan_tx_pdu(txdesc_idx, cs_idx);  \
	}
// modified for rom 20200226
...
extern void (*pf_dbg_trc_req_acl_rx_pdu)(struct lld_acl_rx_ind const* msg);
extern void (*pf_dbg_trc_req_acl_tx_pdu)(const uint8_t trc_acl_tx_type, const uint8_t txdesc_idx);
...
#endif
#define TRC_REQ_ACL_RX_PDU(msg)          \
	if(pf_dbg_trc_req_acl_rx_pdu)        \
	{                                    \
	    pf_dbg_trc_req_acl_rx_pdu(msg);  \
	}        

#define TRC_REQ_ACL_ACK_TX_PDU(txdesc_idx)                  \
	if(pf_dbg_trc_req_acl_tx_pdu)                           \
	{                                                       \
	    pf_dbg_trc_req_acl_tx_pdu(ACL_ACK_TX, txdesc_idx);  \
	}        

#define TRC_REQ_ACL_PROG_TX_PDU(txdesc_idx)                  \
	if(pf_dbg_trc_req_acl_tx_pdu)                            \
	{                                                        \
	    pf_dbg_trc_req_acl_tx_pdu(ACL_PROG_TX, txdesc_idx);  \
	}
